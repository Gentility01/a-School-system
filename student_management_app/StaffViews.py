...
from student_management_app.models import CustomUser, Staffs, Classes, Subjects, Students, SessionYearModel, Attendance, AttendanceReport, LeaveReportStaff, FeedBackStaffs, StudentResult, Term
...
def staff_add_result_save(request):
    if request.method != "POST":
        ...
        check_exist = StudentResult.objects.filter(subject=subject_obj, student=student_obj, term=term_obj).exists()
        try:
            if check_exist:
                result = StudentResult.objects.get(subject=subject_obj, student=student_obj, term=term_obj)
                result.assignment_marks = assignment_marks
                result.test_marks = test_marks
                result.subject = exam_mark
                result.save()
                messages.success(request, "Result Updated Successfully!")
                return redirect('staff_add_result')
            else:
                result = StudentResult( term=term_obj, student=student_obj, subject=subject_obj, exam_marks=exam_mark, assignment_marks=assignment_marks, test_marks=test_marks)
                result.save()
