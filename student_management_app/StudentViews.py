...
from student_management_app.models import CustomUser, Staffs, Classes, Subjects, Students, Attendance, AttendanceReport, LeaveReportStudent, FeedBackStudent, StudentResult, Term
...
def student_view_result_term(request, id):  
    ...
    results = StudentResult.objects.filter(term=term, student=student)
    number_of_subjects = Subjects.objects.filter(term_id=True).count() 
    
    student_result_count = Students.objects.annotate(num_student=Count('studentresult'))
    student_result_count[0].num_student
    print(student_result_count, 'ppppppppppppppppppppppppppppppppppp')
